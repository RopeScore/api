...
import { type ScoresheetDoc, type CategoryDoc, type CompetitionEventLookupCode, isTallyScoresheet } from '../store/schema'
import { NotFoundError } from '../errors'
import { logger } from './logger'
import { Timestamp } from '@google-cloud/firestore'

function filterLatestScoresheets <T extends Pick<ScoresheetDoc, 'createdAt' | 'excludedAt' | 'competitionEventId' | 'judgeType' | 'judgeId'>> (scoresheets: T[], cEvtDef: CompetitionEventLookupCode): T[] {
  return [...scoresheets]
...
export async function calculateResult (categoryId: CategoryDoc['id'], competitionEventId: CompetitionEventLookupCode, { dataSources }: CalculateResultOptions): Promise<{ maxEntryLockedAt: Timestamp | null, results: OverallResult[] | EntryResult[] }> {
  ...
  const models: Record<CompetitionEventLookupCode, CompetitionEvent> = Object.fromEntries(await Promise.all(competitionEventIds.map(async () => [competitionEventId, importPreconfiguredCompetitionEvent(`${competitionEventId}@${rulesVersion}`)])))

  // Get all locked entries for competitionEventId || overall.competitionEventIds
  const entries = (await dataSources.entries.findManyByCategory({ categoryId, competitionEventId: competitionEventId.length === 1 ? competitionEventIds[0] : undefined }))
  ...
  for (const competitionEventId of competitionEventIds) {
    const latestScoresheets = filterLatestScoresheets(scoresheets, competitionEventId)
